...
interface SourceMapData {
  sources: { name: string; content: string;  Int32Array; dataLength: number, mappedByteCount: number }[];
  names: string[];
   Int32Array;
...
let analyzeSourceMapTree = (sourceMapData: SourceMapData): Tree => {
  ...
  let rootNode: TreeNodeInProgress = {
    name_: sourceMapData.file, // Use bundle filename as root name
    inputPath_: '',
    bytesInOutput_: 0,
    children_: {},
  ...
  let sortChildren = (node: TreeNodeInProgress, isOutputFile: boolean): TreeNode => {
    ...
    let name =  node.name_ //commonPrefix ? splitPathBySlash(node.name_).slice(commonPrefix.length).join('/') : node.name_
    return {
      name_: name,
  ...
  for (let sourceIndex = 0; sourceIndex < sources.length; sourceIndex++) {
    ...
    accumulatePath(rootNode, source.name, source.mappedByteCount);
  }


  return {
    root_: sortChildren(rootNode, true),
...
export let createTreemap = (sourceMapData: SourceMapData): HTMLDivElement => {
  ...
  setWheelEventListener(e => {
    if (isWhyFileVisible()) return // Adjusted path
    updateHover(e)
  })
